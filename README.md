...
Observation-Areas to keep an eye on:
-

Old-topics people need to homework:
- 
...
# Introduction:
The year 2024 is a fascinating topic, though perhaps not for reasons we might at first suspect. Starting in late 2022 a set of events slowly started snowballing (OpenAI's releases of ChatGPT version 2-4, which by all accounts were becoming significantly, and unexpectedly, improved). 

In 2022, 'AI' was not a hot topic.

In 2023, it seemed that everyone was predicting that 'AI' had arrived and would rapidly either destroy or otherwise reshape the world, where events were happening too quickly for academia, journalists, companies, trade associations, governments, internet-discussions, and regulators, to do more than scramble to both make and watch headlines.

2024 should, reasonably, have been a year where various world participant institutions (from academia to regulators, etc.) formed some manner of apprehension of what was going on (however imperfect that rendition might later be proved to be). But...somehow that did not happen.

So we have at least two layers to the question of what happened in 2024. We have our original question, "What was the state of AI-ML technology in 2024?", and we have a context of questions around societies' reactions and non-reactions to that: "What were people saying?" and "How and why were people not saying things that indicated a serious recognition of the question?"

We have...a not short investigation in front of us. To some extent it can be mapped out into a few most-common areas. But there are also a significant number of other areas of concern and investigation. 
...
-- task decentralization and distribution
-- project object databases
...
- clearweb-deepweb:
	...
	- safe ~municipal/public-sector use
...
- Broad STEM Scope vs. Narrow Discussion Scope
...
- From 1800's Modernity-drama to 1940's Norbert Weiner & Gorge Orwell: Why project-management, psychology, learning, and decision-making matter.
...
A. are mass-market big game companies firing human developers and simply using 'ai bots' to make games instead? 
...
- specific vs. 'general' (largely undefined)
- structured-data vs. unstructured-data
- db-search vs. vector-search vs. guestimation
...
Is throttling safety-sensitive issues an issue for doing safe work?
- safety standards for software
- encryption
- safely navigating copyright
The age-old circular anti-security bottlenecks.
...
- 2x increase is...not an exponential hockey-stick.
- this is a space very 'ripe for disruption'


...

Topic: Confusing System-1 and System-2, Misunderstanding fuzzy-guestimation

Again, it may be helpful to look at concept-matrix generative output as a form of 'vector search.'

As illustrated by this article:
https://www.economist.com/science-and-technology/2026/01/05/an-ai-revolution-in-drugmaking-is-under-way 

This article dangerously, irresponsibly, inadvisedly, confuses, conflates, and mixes-up, fuzzy guestimation generative transformer models as compared with (vs.) brute force deterministic simulation-modeling.

Adding to this soup of incompatible jargon the extremely fraught term 'agent' (which the CEO of anthropic has specifically called out as an arm-waving term of deception that has no STEM meaning at all) 

This lack of understanding in ignorance, or willing act of disruptive disinformation to defraud readers, customers, and civilians, also relates to the general topic (and recurring project issue) of interfacing structured and unstructured information, which is very important to understand for anyone designing a data-science and or machine-learning system. 
...
Naufal Suryanto1, Muzammal Naseer1, Pengfei Li1, Syed Talal Wasim2,
Jinhui Yi2, Juergen Gall2, Paolo Ceravolo3, Ernesto Damiani3
...
(no one has any idea, even about what they are trying to measure)
...
There is also the ambiguity between 'a model' vs. a larger architecture of moving parts and possibly several (possibly not static) models. A given academic BERT model is something that can be studied and compared and the results peer-analyzed and replicated, but we may never know what various versions of GPT-4 or Gemini were. What are described, for example in academic-looking papers (that cannot be peer-reproduced or clearly decoded), are "models" such as GPT-N or Gemini-N, but it is probable that these are 'cloud services' that no doubt make use generative technologies, but the scale can diversity of an international cloud service with load balancing layers and optimization layers and content filtering layers and security layers etc., are likely not simplistically equivalent to a single static model. Or, perhaps in some cases it is as simple as one static model load-balanced for many users, but it is unhelpfully unclear what was what in 2024. (e.g. there was a lot of anecdotal talk about gpt-4-service being a mixture-pipeline of separate expert models (which may or may not have been one mixture-model as was "Mixtral", however that type of model at least publically did not prove to be the most performant). Too much like Deep-Blue in 1997, we probably won't ever know what happened in 2024 and not-knowing is not how science and science progress work. Consistently with that, both deep-blue and gpt-4 were followed by years of backsliding confusion, not world-helping transformative STEM.
...
## Throttling & Safety vs. Overall Performance: Soggy Performance

An offshoot topic (from the above topic of what exactly were the 2023-2024 so called 'models') is that during 2023-2024 there was an (at the time discussed and documented, but later not discussed) phenomena of throttling the models because they were too good and thought to be potentially dangerous.
There is an excellent video by a researcher with open-air describing how there was a very significant drop in performance between early versions of gpt-4 and later versions, due to safety-throttling of the model to prevent people from asking dangerous questions. In throttling the model it did not just limit select topics but overall made all performance profoundly lower.
(find source)
...
The subsequently revised Bubeck paper is here: https://arxiv.org/abs/2303.12712
The archive article was submitted on 22 Mar 2023 (v1), last revised 13 Apr 2023 (this version, v5). This lecture was posted Apr 6, 2023, as of Feb 2nd 2026, Dr. Bubeck's most recent post is from Sep 11, 2023. This is a classic example of the strangeness of the landscape we are trying to make sense of. As nothing of any use has been developed from OpenAI's Hype-Fest in late 2022, there has been literal silence from experts in the field. 


This topic too has become mired in confusion. From historically 'fixed' wrong gemini image photos of fake-histories to X-grok-ai's attempts to be unfiltered but that are especially top-performing, to the overall sag in performance, and the inverse of Anthropic's most-throttled models also being the best-performing for many or most tasks... and all the while academia is both out of the loop but also producing a diversity of peripheral research (e.g. see archive) that also somehow does not translate into anything usefully applied (not that research should be measured by profitable applications). 

And, as another example, while all this is going on, through 2025 and into 2026 most work-meetings and job interviews where people are supposedly trying to 'build first in class AI services' refuse to discuss anything beyond SQL queries and high level third party low-code-no-code platforms, and have zero acknowledgement or interest in any of these topics. 

The situation in early 2026 is extremely unclear, aside from the very prominent 'low bar' pattern that very very few people care about this topic at all what-so-ever, including people in the IT industry and supposedly making and using this technology.
...
## Segue
A variation on the mismatch between the fear that gpt3 to gpt4 would either take over the world or allow criminals to take over the world, is a perhaps more skynet type military-AI race that notably did not happen (so far as there is any evidence) through the stretch of 2023, 2024, 2025. 

Where the context here is trying to create a portrait of what was happening (including in people's perceptions) in 2024, the still largely ambiguous land-invasion by the Russia-China-North-Korea group has continued escalating to a kind of situation for which there is not a clear term. It is not a full hot war as in after official declarations of war in WWII (though the western front was notoriously not 'hot' for a strangely long time). It is not a traditional cold war either. And the new domains of 'hybrid war' 'information war' and 'grey-zone tactics' also do not fit traditional modes of 'war' 'cold war' or 'peace.' 

There has been (from 2022 into 2026) a perhaps perfect opportunity for nationstates with deep pockets to have gone into an AI-arms-race with military fronts, so that each side's ever-better AI was achieving novel military victories and surprises. By and large this has been either absent or lurking out of sight, and not being discussed, while at the same time the world is in this strange AI-limbo, making all kinds of predictions while an opaque nothing continues on and on.
...
https://www.economist.com/britain/2026/01/15/britains-high-tech-hunt-for-russian-subs-in-the-north-atlantic 
...
On a separate but perhaps related theme, AI is also very stubbornly very bad about basic coding best practice. 

I have been evolving a standard coding best practice requirements system-instruction / prompt-header to cover the areas of standard unacceptable failures. That list of unacceptable failures is currently 8-12 pages long (depending on if a code example is included, because the instructions need to be programming language-specific), which is longer than the entire input 'context' window of models from a few years ago. And this is sufficiently long that it runs into various 'long prompt' issues.
1. models and sizes of models react very differently to prompt length alone (some like or need longer prompts, some cannot handle long prompts at all). Even where long prompts are allowed, there is an inevitable attention-span scattering where the focus of the model (e.g. on what it is supposed to be doing) is diluted. 

2. Each version of a model (constantly changed in cloud services) can react completely differently to the same prompt. This is a serious issue for 'production tool' design. 

3. A larger problem is that the training data overwhelmingly represents bad-practice in coding and how that inevitably overwhelms any prompt no matter how detailed. 
...
the state of hiring and admissions
..
...
training data and "stack overflow" type issue:
...
Topic: Broad STEM Scope vs. Narrow Discussion Scope
https://arxiv.org/abs/2601.09925 
https://arxiv.org/abs/2601.09941 

...
Salesforce announces Pullback from AI due to reliability issue after bragging about replacing 4k devs with magic inhouse AI.

https://www.youtube.com/watch?v=TbIPUVkImGE
Salesforce Admits they were Wrong
"The PrimeTime"
https://www.nbcbayarea.com/video/news/local/salesforce-4000-layoffs/3942259/ 

https://www.cxodigitalpulse.com/salesforce-clarifies-ai-strategy-denies-layoffs-amid-claims-of-replacing-staff-with-ai-agents/ 

https://economictimes.indiatimes.com/news/new-updates/ai-bubble-bursting-salesforce-execs-admit-trust-issues-after-laying-off-4000-techies-now-scaling-back-use-of-ai-models/articleshow/126139465.cms?from=mdr 

https://maarthandam.com/2025/12/25/salesforce-regrets-firing-4000-staff-ai/

https://www.moneycontrol.com/technology/salesforce-pulls-back-from-large-language-models-after-reliability-concerns-article-13738832.html 


This is not one of The Economists better vetted articles:
https://www.economist.com/business/2026/01/12/job-applicants-are-winning-the-ai-arms-race-against-recruiters 


bloomberg quip:

Scarlett Foo and Ken Tucker 2026 jan 21:
"""
~"Ken Tucker: Hundai has overtaken GM as the world's 4th most valuable automaker. And you can credit to some extent their push into robotics, remember they bought boston dynamics business and unveiling the latest humanoid robot it's designed to work in their manufacturing plants, the car manufacturing plants, starting in 2028, 
Scarlett Foo: Increased manufacturing efficiency at the factory, in other words.
Ken Tucker: That's the idea, but does it work? Or does the robot [pause] turn into an evil monster?"
"""

...

Analogy Case Studies:
- Microsoft, Internet Explorer, IE Integration Frenzy, Zombie IE...


...
Austen’s profile photo
Austen Allred commented on this


View Tyler Pohn’s  graphic link
Tyler PohnTyler Pohn
   • 2ndPremium • 2nd
Senior AI Software Engineer at FSP, Founder @agentseo.org, Gauntlet AI Champion 🛡Senior AI Software Engineer at FSP, Founder @agentseo.org, Gauntlet AI Champion 🛡
21h •  21 hours ago • Visible to anyone on or off LinkedIn

Pending
You have already invited Tyler Pohn
I built a 200M-parameter AI that plays competitive Pokemon.

The project combines modern transformer architectures with behavioral cloning to learn from 20,000+ real competitive battles.

Architecture:
- Decoder-only causal transformer (GPT-style)
- 14 layers, 1024 hidden dim, 16 attention heads
- RoPE positional encoding for turn-based reasoning
- SwiGLU activation for efficient compute
- HL-Gauss value head for win probability estimation
- Flash Attention + KV caching for real-time play

Data:
- 20,100 Pokemon Showdown replays (Gen 9 OU/UU/RU/NU)
- 867,945 training samples from 39,998 trajectories
- 613 unique Pokemon species
- Real competitive battles, not random play

Training:
- Behavioral cloning from expert demonstrations
- Mixed precision (BF16) on NVIDIA A10G
- Cosine LR schedule with warmup

The model learns strategic concepts like:
- Type matchups and damage calculation
- When to switch vs. attack
- Team preview decisions
- Late-game win condition identification

This is beating real human players. 

hashtag#MachineLearning hashtag#GameAI hashtag#Transformers hashtag#DeepLearning hashtag#Pokemon
...


# Unpredictable Future

Part of what I like about this (however imperfect) article,
The Economist: "Will the smartphone survive the AI age... ~The Apple-Android duopoly"
https://www.economist.com/business/2026/01/25/will-the-smartphone-survive-the-ai-age 

is that it is able to ~optimistically hint at the future being unlike the present, and somewhat describes the difficulties of creating and investing in new technologies.

Something that I think should supplement this article is a bit more history and historical context for the mobile space of early 2026.

1. The Prehistory of the iPhone: Three Decades of Mobile Devices in Japan 
2. The broad embrace of the low-tech high-abstraction general iphone-mobile device model 
3. 2008-2018 Slight Over-reach of the mobile model: tablets vs. laptops
4. Windows 8.1
5. AR/VR's slow rise 
6. Rabbit AI's R1 
7. Significant Backend & 'Sub-feature' overhead.
8. Touchscreens are a nightmare that never ends.

Timeline of predictions: if it took mobile devices 40 years to reach equilibrium, with the last twenty years being recognizable, how do those timescales apply to predictions about a supposed new-new technology? E.g. If that can be expected to undergo forty years of changes and not be stable at all in form for 20 years...what kind of near-term or long term predictions are we trying to make?

The click-bait fantasy of ignoring older working technology:
- Server-farms still exist.
- Desktops still exist.
- Portable laptops still exist.
- Tablets are dubious.
- Mobile devices (phones) are becoming more entrenched (payment, ID, flight-checking, banking-login, etc.)

See: 'The mother of all demos'

The Global South and Developing Markers
...
- How to remove the AI startup market from itself:
...
An inarticulate article that fails at cynical humour and end up blundering nihilism:
...
This is overall a better than average discussion, not 5-sec rage-bait,
but it is significantly vague and misleading in several areas.


...

Tabular and IoT Data vs. ChatBots
https://www.amazon.com/Sentient-Enterprise-Evolution-Business-Decision/dp/1119438861 

...


Google: Genie 3
https://www.youtube.com/watch?v=cEPTbXuw55Q
...
(not a good article)
https://www.economist.com/business/2026/01/29/dont-be-fooled-ai-bosses-are-regular-capitalists 


...


The high level of self-contradictory nonsense in this article (being reported on, if also in the reporting) is problematic. 
https://www.economist.com/business/2026/02/01/why-software-stocks-are-getting-pummelled 
'still subsidized'
'utter chaos' in investment

A profoundly inadequate description of AI code: 'elegance-ness'? This is not acceptable. 
...
On Dec. 8 and 10, 2025, the Mars rover completed drives planned by AI – not human mission planners.


In a first-of-its-kind demonstration, generative AI created navigation waypoints for the rover, a complex decision-making task typically done by teams on Earth.


Teams at JPL leveraged a type of vision-capable generative AI called vision language models (VLMs) to analyze existing data from our vast surface mission dataset. The AI used the same imagery and data human planners use to generate waypoints – fixed locations where the rover takes up a new set of instructions – so that Perseverance could safely navigate the challenging Martian terrain.


To ensure the AI's instructions were fully compatible with the rover's flight software, the engineering team also processed the drive commands through JPL’s “digital twin” (virtual replica of the rover), verifying over 100,000 telemetry variables before sending commands to Mars.


The successful demonstration hints at a future where planetary exploration is more efficient and delivers even more science.


“That is the game-changing technology we need to establish the infrastructure and systems required for a permanent human presence on the Moon and take the U.S. to Mars and beyond," said Matt Wallace, manager of JPL’s Exploration Systems Office.


Learn more: https://lnkd.in/gkH8YKUs
...
cult?:
https://www.youtube.com/watch?v=WNXbRyA1JC4 

...


Lack of agreement about 'reasoning' and 'chain of thought'
https://arxiv.org/abs/2602.02496 


...
???
https://arxiv.org/abs/2602.02520 
...
System engineering still in the margins.
...
...
As people under the ELIZA-Effect keep saying 'AI will replace everything.' Maybe I was misinterpreting that...

Steve Gibson: AI irreversibly deletes all project files. Says it's sorry.
https://www.youtube.com/watch?v=bwRsUtUp9n8 
...
The state problem, and the brilliant but limited fudge of GPT
- https://arxiv.org/pdf/2602.06976 

GPT(descriptively meant) is more successful than past 'knowledge graph' type systems. But it is unclear what methods for what cases can work well enough to bridge stateful discreet, specific-environment and specific-task navigation, compared with a generally fuzzy guestimation that is very stateless.

...

https://arxiv.org/pdf/2602.06980 
```
"Based on a closed-door roundtable held during the 2025 Society of Toxicologic Pathology (STP) Annual Meeting and follow-on discussions, this paper synthesizes the perspectives of leading toxicologic pathologists, toxicologists, and AI developers. It outlines the key pain points in current reporting workflows, identifies realistic near-term use cases for agentic AI, and describes major adoption barriers including requirements for transparency, validation, and organizational readiness. A phased adoption roadmap and pilot design considerations are proposed to help support responsible evaluation and deployment of agentic AI system in nonclinical settings. The paper concludes by emphasizing the need for coordinated efforts across pharmaceutical organizations, CROs, academia, and regulators to establish shared standards, benchmarks, and governance frameworks that will lead to safe, transparent, and trustworthy integration of AI into toxicologic science."
```

...
linkedin post 2026.02.11
...
The good news for employees is that most businesses have a lot of work to do and not enough people to do it. People with the right AI skills are often given opportunities to step up and do more.
"""
...
Strawman-Modernity: Anti-Tech Rage
...
Utterly illiterate article, which dangerously reinforces a potemkin facade of word-salad.
...
Using 'AI' for border-enforcement that needs to have human oversight:
- tools for people
- automatic tools with no oversight
- refusal to report on false-positives
...
(Note 2024-workshop findings published in 2026... in AI-research time-spans...a LOOOOOOOOONG time ago.)
https://link.springer.com/article/10.1007/s00204-025-04286-8 
Building trust in AI for chemical risk assessment – our new open-access article is now published in Archives of Toxicology.

Artificial Intelligence is rapidly transforming chemical safety assessment, enabling faster, more human-relevant, and more predictive evaluation of chemical hazards and risks. But as AI becomes more deeply integrated into regulatory science, building trust, transparency, and scientific robustness is essential.

In this new ECETOC workshop report, we explore how AI, both predictive and generative, can support chemical risk assessment, while highlighting the critical role of:
• FAIR data principles as a prerequisite for trustworthy AI
• Transparent, explainable, and validated computational models
• Human oversight and governance frameworks
• Knowledge infrastructure supporting reproducibility and regulatory acceptance

AI offers unprecedented opportunities to integrate chemical, biological, and environmental data to improve safety assessment and accelerate the transition to Next Generation Risk Assessment (NGRA) and animal-free toxicology. However, as the workshop concluded, responsible implementation requires robust data governance, structured workflows, and collaborative international frameworks. 

This work reflects a collective effort across academia, industry, and regulatory organisations to advance trustworthy AI in regulatory science.

Open access article:
https://link.springer.com/article/10.1007/s00204-025-04286-8

I’m grateful to our co-authors and colleagues across ECETOC and the wider community contributing to this important and rapidly evolving field. 

hashtag#AI hashtag#Toxicology hashtag#ComputationalToxicology hashtag#RiskAssessment hashtag#NGRA hashtag#FAIRdata hashtag#Regulatory hashtag#Toxicology hashtag#Workflow hashtag#ASPA hashtag#Governance hashtag#Knowledge hashtag#Infrastructure hashtag#NAMs ECETOC: European Centre for Ecotoxicology and Toxicology of Chemicals Timothy Gant, Alistair Boxall, Daniel Burgwinkel, Maryam Zare Jeddi, Ivo Djidrovski, PhD, Steffi Friedrichs, Barry Hardy, Thomas Hartung, Daniela Holland, Andreas Karwath, Anne Kienhuis, Nicole Kleinstreuer, Zhoumeng Lin, Emma Marczylo, Antonino Marvuglia, Hua Qian, Bennard van Ravenzwaay, Paul Rees, Haralambos Sarimveis, Tewes Tralau, Lucy Wilmot, Adam Zalewski David Rouquieé
...
On Languages...
https://www.youtube.com/watch?v=MwMaBg7JpDc 
Note: Pay attention to how the term 'symbol' is or is not clearly defined; the term 'symbol' itself likely represents a rhetorical abstraction that is a barrier by way of being both undefined and strongly assumed by most to have been defined and to be widely understood (assuming the Emperor has clothes somehow, though you can't see them, because everyone else acts like they do see them). E.g. 'symbolic' vs. 'sub-symbolic' AI (a topic since at least the 1940's) approach to learning (which kept Hinton's research taboo) is a mire of rhetoric and it is non-trivial to parse out what is really defined and what is a stack of turtles in a hall of mirrors.

...

Leigh-Ann Russel, CIO
ELIZA
BNY

...

arXiv:2603.02218 [pdf, html, other]
...
questionable: some references may be useful, but others are clearly absurd: e.g.
...
Billions of discrete AI-beings "in" a "prompt"? 
https://www.economist.com/interactive/briefing/2026/08/20/the-search-for-consciousness-inside-llms
- note: this does not mention language and concepts once

..

This is a typically horrible article that throws around vapid century old cliche clickbait tropes. 
...
AI focused weekly edition:

https://www.economist.com/weeklyedition/2026-08-22

..

(if surprisingly) good (general) discussion of types of projects and code:
...
The Sociology of Reification and Abstraction:



Task-Set Derivation and Task-Set Integration

2026.08.24
Google search: ""does mcp sometimes increase token usage and sub-task number"

```google-search-result 
Yes, using the Model Context Protocol (MCP) frequently increases both token usage and the number of sub-tasks. Loading multiple verbose tool schemas upfront and processing large, raw API outputs bloat the context window, while a high volume of available choices often causes AI agents to break work into extra steps.Causes of Increased Token UsageSchema Bloat: Connecting multiple or poorly optimized MCP servers injects thousands of tokens of tool definitions, names, and JSON schemas into the prompt before the agent even starts working.Response Bloat: Tool outputs return large blocks of raw data or logs directly back into the context window, consuming massive amounts of input space on subsequent turns.Causes of Increased Sub-TasksChoice Confusion: Exposing dozens or hundreds of tools makes it harder for the model to pick the right function, triggering trial-and-error loops.Granular Execution: Agents with complex tool sets often break a simple objective into many small, discrete sub-steps rather than handling tasks efficiently.If you want, I can share ways to optimize your MCP setup to cut down on token waste and reduce extra steps.
```

The era of "token-maxing" and adoption of MCP:
- What are the incentives?



- https://arxiv.org/html/2602.14878v3 
...
yahoo news: Actually, MCP wastes a lot of tokens when compared 

the new stack: 10 strategies to reduce MCP token bloat

myclaw: The MCP Token Trap: Why MCP Costs 35x More Than CLI ...
...
Google Cloud Tech
 and Smitha Kolan - Machine Learning Engineer
"""

@viktorlza
1 month ago
This feels like it could have been easily solved with an OpenAPI (not OpenAI) spec for the existing API.
...
@penaplaster
1 month ago (edited)
Wait a second. Good APIs come with the OpenApi specification file. That spec does exactly what MCP does: describes what operations available, what they do, what the parameters are, etc. Point your agent to the OpenApi specification and it will understand what endpoints to call and what parameters to pass there. It seems that with MCP we’re repeating what’s already there. Could you please elaborate on this?
...
@bonafernando
1 month ago
100% I had the same question. It's all so beautiful on theory, but feels like it's just another layer of text on top of what already exists to help AI. If the docs are really good, this should already exists. Shouldn't it?
...
@BobbyBundlez
1 month ago
All MCP is truly is a manual on how to access an API. This video is so dumb. MCPs are not even that efficient.
They are NOWHERE near as efficient as an API lol
...
@TheCstriker
1 month ago
MCP is the biggest token black hole, some providers started supporting dynamic tool search to stop the token chug, it is preferred to use a SKILL that talks to a CLI (github MCP is the best example vs gh cli), as the SKILL only consumes the frontmatter upfront, leaving the rest for when the skill is actually used, while MCP loads every skill into the context
...
@Foreversun33
1 month ago
The important thing is that you use as many tokens as possible; the rest is just a detail.
"""
